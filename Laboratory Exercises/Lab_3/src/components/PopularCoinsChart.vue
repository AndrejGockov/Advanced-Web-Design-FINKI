  ...
  for (const coin of topCoins.value) {
    try {
      ...
      if (chartArray && chartArray.length > 0) {
        ...
        errors.value[coin.id] = `No data available for ${coin.symbol}`;
      }
    } catch (error) {
      console.error("Error fetching chart data for " + coin.id, error);
      errors.value[coin.id] = `Error fetching data: ${error.message}`;
    }
  }
};
...
<template>
  <div className="bg-[#1B2028] rounded-[10px] p-[20px] w-[1000px] flex flex-wrap justify-around">
    ...
    <div class="w-full text-center mb-4">
      <label for="timeInterval" class="text-white mr-2">Select Time Interval:</label>
      <select
          id="timeInterval"
          v-model="selectedCoinData"
          @change="fetchChartData"
          class="bg-darkBlue text-white px-4 py-2 rounded border border-lightGray"
      >
    ...
    <div v-for="coin in topCoins" :key="coin.id" className="w-[45%] mb-6">
      ...
      <p v-else class="text-white text-center">
        {{ errors[coin.id] || `Error fetching chart data for ${coin.symbol}` }}
      </p>
    </div>
  </div>
</template>